...
Principal components analysis (PCA) is a statistical technique often used for data reduction. PCA is often used when a researcher collects multiple dependent variables that are highly correlated with each other and are somehow related. PCA can also give some understanding into how different groups of variables are related to each other. In this challenge question, we ask you to perform a PCA on the following variables in the `WorldBank` data set: `fertility.rate`, `life.expectancy`, `population`, `GDP.per.capita.Current.USD`, and `X15.to.25.yr.female.literacy`. Many of these variables are correlated with each other, and thus we could potentially use principal components analysis to reduce the number of variables we are looking at by considering some overall "socioeconomic variable". You should perform a PCA on these variables, assess the results of the PCA, and then produce a biplot for the first two principal components showing how the different regions are related to each other in terms of these components by plotting the points with different colors for each region. We took the following steps: 

1. Install and load the `ggbiplot` package for producing a biplot. Note: this will also install a couple other packages if they aren't already installed, including `ggplot2`. 
2. Subset the `WorldBank` data set to only grab the variables you will include in the PCA, as well as the `region2` variable. You will also want to remove any rows of these variables that contain any NA values (as PCA does not go well with NAs). I find the `na.omit` function useful for this. 
