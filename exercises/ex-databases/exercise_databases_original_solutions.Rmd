...
In this exercise we will work with `MonetDBLite`, a database that runs inside the R environment and does not require the installation of any external software. It is based off of the free and open-source `MonetDB` and works in a similar manner. One useful aspect of `MonetDBLite` is that it can complete queries extremely fast as a result of its columnar storage structure. We will install the pacakge from the GitHub repository where the package is regularly updated. This requires the use of the `devtools` package, so if need be, install `devtools` using `install.packages("devtools")` before running the following line of code to install `MonetDBLite`.
...
First we need to set up an area on our hard drive where we can store our database. Set the variable `dbdir` to "myDBDir". This will create a new folder in your current directory called `myDBDir` that will contain the database and the tables we will use. We will then connect to the `MonetDBLite` database using the `dbConnect()` function from the `DBI` package. 
...
It is important to note here that MonetDBLite allows multiple concurrent connections to signle database, but does not allow more than one concurrent embedded server session. This could lead to some problems when you are working on this part of the exercise if you have the Shiny app running at the same time you are trying to make edits in the R Markdown document. If you encounter some weird errors suggesting MonetDB has failed or there is a lock on the database, you can shutdown the server using 
...
To run the app, press the `Run App` button in the top right of the script window in RStudio. Mess around with the user interface to see exactly how the app works. 
