...
In this exercise we practice using `ggplot2` functions for creating graphics. Some of what you'll be doing is not covered explicitly in the book, so you will probably find the `ggplot2` help at https://ggplot2.tidyverse.org/reference/ to be useful, as well as other references provided in Chapter 5. Of course Google is also quite helpful!
...
```{r p-col-lm, echo=FALSE}
p + geom_point(size=1, aes(color = continent)) + stat_smooth(method = lm, se = FALSE) + labs(title = "TODO 6.2: plot 2")
```

The least squares line is not very visible. Make it wider. Also change its color to green. Here is what the graphic should look like (Note: we won't take off if your line width and color do not *exactly* match ours, but do try to get them as close as possible):

```{r p-col-lm-grn, echo=FALSE}
...
```{r p-col-lm-grn-cont, echo=FALSE}
...
```{r p-col-sm-grn-cont, echo=FALSE}
...
So far the focus has been on life expectancy versus per capita GDP. Next we investigate how life expectancy changes over time, and limit our attention to one or a few countries. First, here is the graphic for Rwanda (do you recall what happened in Rwanda in the early to mid 1990s). Recreate this graph. 

```{r, echo=FALSE}
ggplot(data = subset(gapminder, country == "Rwanda"), aes(x = year, y = lifeExp)) + geom_line() + labs(title = "TODO 6.6: plot 6")
```

Probably adding the actual points being connected by the line segments would add clarity. Add the points to the graph as in the following: 
...
Consider the distribution of life expectancy. Here is a histogram of life expectancy (Note: you will likely receive warning messages. These messages are fine and just provide suggestions regarding the number of bins to use).
...
`ggplot2` can produce some really fantastic, publication quality plots. As previously showed, creating facet plots is a nice way to group sets of plots together. However, there are additional methods for grouping multiple plots in the same row, column, or a table like format, which are tasks that are often desired when preparing a publication. In this question, you will use the `ggpubr` package to create a grouping of three plots of the gapminder data set. You should produce the following plot. Here are a couple tips for recreating these plots: 

1. Install and load the `ggthemes` and `ggpubr` packages, two very useful extensions of `ggplot2`. 
2. Use the `scale_color_colorblind()` function in the first plot to create a color palette that is easily visible for colorblind folks like Jeff. This color palette is contained within the `ggthemes` package.
3. Use the `theme_bw()` theme for the first graph and the `theme_classic()` theme for the second graph. 
4. Use the `ggarrange` function to arrange the three plots in vertical format, giving them labels of A, B, and C, respectively. 


```{r, echo = FALSE, fig.height = 12, message = FALSE}
library(ggthemes)
library(ggpubr)
plot.1 <- p + 
...
If you have any lingering questions about the material in this document or in the corresponding chapter, put them at the end of your R Script.

### Give us your feedback!

Place answers at end of R Script.
