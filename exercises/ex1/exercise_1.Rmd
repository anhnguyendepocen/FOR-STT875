...
ls()
```
Initially we'll be interested in the ten objects that contain the batting data set, number of home runs, and number of RBIs for three players, Jim Rice, Carlton Fisk, and Ted Williams. The other object we read in, `batting_stats`, contains such data and more for a wide variety of players. 
...
6. What was the largest jump in Ted Williams' RBIs from one season to the next? In which season did this jump occur? Note that we are asking for the largest *magnitude* jump, regardless of whether or not it was an increase or a decrease.

Congratulations! You finished Exercise 1. 
...
<!-- Variables in data frames are represented by columns. We can find out the names of the variables using the `names` function. Also the `dim` function yields the number of rows and columns of the data frame. We can also select particular rows and/or columns as shown below. Below we provide an example using the `batting_stats` data frame.  -->
...
<!--   8. Display the player name, batting average, number of home runs (HR), number of RBIs, and year for Lou Gehrig. -->
...
<!--   13. Try to extract the rows when the number of home runs is greater than 90. What happens? -->
