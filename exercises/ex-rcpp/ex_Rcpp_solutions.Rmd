...
*  `all` - Given a logical vector, are all of the values true? Return `TRUE` or `FALSE`. Write `allC`.
*  `range` - Return a 2-item vector containing the minimum and maximum of a numeric vector. Write `rangeC`. 

*Note*: C++ does have a `max()` function, but it is only for pairwise maximum (two arguments), not vector maximum element. Think about how you can iterate through a vector to find the max and min values. (*Hint*: Look at the textbook's [Vector input and scalar output](https://www.jeffdoser.com/files/for875/_book/rcpp.html#vector-input-and-scalar-output) section code for a loop example.)
...
Below I wrote an R function called `distR()` that computes the inter-site Euclidean distance matrix for points defined in a n-by-p matrix, where n is the number of sites and columns define the coordinate values in p-dimensional space. Your task is to rewrite my `distR()` function using an Rcpp `cppFunction()` function called `distC()`. Then give the timings of the two functions, i.e., distR() and distC(), using `microbenchmark()`. Choose test cases that convince you that your Rcpp function is more efficient.
